        ...
        <div className="w-full bg-white p-3 md:p-4 rounded-3xl shadow-xl border border-blue-100 relative overflow-hidden group hover:shadow-2xl transition-shadow duration-300">
          <div className="relative flex items-center py-1 md:py-2">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && !isAnalyzing && onAnalyze()
              }
            ...
            <button
              onClick={onAnalyze}
              disabled={isAnalyzing}
              className="absolute right-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-10 py-4 rounded-2xl text-lg md:text-xl font-bold shadow-md transition-all transform hover:scale-105 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed min-w-[165px]"
            >
              {isAnalyzing ? `분석중${loadingDots}` : "분석하기"}
            </button>
          </div>
        </div>
      </div>
